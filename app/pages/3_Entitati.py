"""Pagină: Entități — timeline NER + sentiment per entitate × perioadă."""
...
st.set_page_config(page_title="Entități și Sentiment", page_icon="👥", layout="wide")

st.title("Entități și Sentiment")
st.markdown(
    "**NER cu GLiNER multi-v2.1 (zero-shot)** pentru persoane politice, țări, instituții + "
...
st.markdown("**Numărul de mențiuni** ale fiecărei entități în discursul lui Nicușor Dan, pe trimestru.")
...
plt.colorbar(im, ax=ax, label="Număr mențiuni")
...
ax.set_title(f"Mențiuni entități × perioadă ({projection})")
fig.tight_layout()
st.pyplot(fig)
plt.close(fig)
...
if sentiment.empty:
    ...
    st.markdown("**Tabel detaliat** (cu filtre):")
    ...
    sent_display = sent_display.drop(columns=["period"]).rename(
        columns={"period_short": "perioadă",
                 "entity": "entitate",
                 "n_passages": "paragrafe",
                 "n_docs": "docs",
    ...
    entity_filter = st.multiselect(
        "Filtru entitate",
        options=sorted(sent_display["entitate"].unique()),
        default=[]
    )
    if entity_filter:
        sent_display = sent_display[sent_display["entitate"].isin(entity_filter)]
